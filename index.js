const uploadForm = document.querySelector(".file-upload__form"),
  ...
  faliureContainer = document.querySelector(".faliure-container");
let correct = true;
...
function uploadFile(name) {
  ...
  xhrRequest.upload.addEventListener("progress", ({ loaded, total }) => {
    ...
    if (fileSize.split(" ")[0] > 200){
        let faliureMarkup = `<div> File Size is greater than allow file size limit</div>`;
        faliureContainer.classList.add("faliure");
        faliureContainer.innerHTML = faliureMarkup;
        correct = false;
    }
